...
function language_fraud() {
    if ($_SERVER['REQUEST_URI'] == "/uk/corporate_education_schedule/") {
        header('Location: '."/corporate-education-schedule/");
        die();
    } else if ($_SERVER['REQUEST_URI'] == "/uk/corporate_education/") {
        header('Location: '."/corporate-education/");
        die();
    }

    if (
        strripos($_SERVER['REQUEST_URI'],"roadmap_order_step1") !== false ||
        strripos($_SERVER['REQUEST_URI'],"b2b_order_step1") !== false ||
        strripos($_SERVER['REQUEST_URI'],"b2b_rent_step1") !== false) {
        return;
    } elseif (
        $_SERVER['REQUEST_URI'] == "corporate-education" ||
        strripos($_SERVER['REQUEST_URI'],"corporate-education") !== false ||
        $_SERVER['REQUEST_URI'] == "corporate-education-schedule") {

        if ($_COOKIE['fix_language'] == 'uk') {
            add_filter('locale', function($lang) {
                return 'uk';
            });
            setcookie('pll_language', 'uk', time()+9999999);
        } else if ($_COOKIE['pll_language'] == 'ru_RU') {
            add_filter('locale', function($lang) {
                return 'ru_RU';
            });
        ...
        switch (get_locale()) {
            case 'uk':
                setcookie('pll_language', 'uk', time()+9999999, "/");
                setcookie('fix_language', 'uk', time()+9999999, "/");
//                $_COOKIE['fix_language'] = 'uk';
                break;
            case 'ru_RU':
                setcookie('pll_language', 'ru', time()+9999999, "/");
                setcookie('fix_language', 'ru_RU', time()+9999999, "/");
                break;
            default:
//                echo 33;
                setcookie('fix_language', get_locale(), time()+9999999, "/");
                break;
        }
    }
...
function hideLangSwitchAndSetCorrectLang($hide = true) {
    if ($hide) {
        add_action('wp_head', function() {
            echo '<style type="text/css">#header .container .lang {display: none !important;}</style>';
        });
    ...
    foreach ($categories as $key ) {
        switch ($key->cat_ID) {
            ...
            case '251':
                ...
                break;
            case '1593': // Графический дизайн  ua
            case '1591': //ru
                $courses_order[8] = $key;
                break;
            ...
            case '1060':
                ...
                break;
            case '1378': // Mobile Development
            case '1380':
                $courses_order[11] = $key;
                break;
            case '985': // Data Science/Machine Learning old
            case '1589': //new
            case '987':
...
//                break;
            ...
            case '1667'://Full Stack Java Developer
                $courses_order[17] = $key;
                break;

            case '1677'://Business analysis
...
<!--                <img src="--><?php //bloginfo('template_directory'); ?><!--/relize/img/icons/calendarIco.png">-->
                <?= ($lang ? 'Перейти к расписанию' : 'Перейти до розкладу'); ?>
            </a>
            <h1><?php single_cat_title(); ?></h1>
        </div>
        <p class="text-test-try">
            <?= ($lang ? 'Не можете определиться с направлением?' : 'Не можете визначитися з напрямком?'); ?>
            <a href="https://pathfinder.jobs/ru" target="_blank"><?= ($lang ? 'Пройдите тест' : 'Пройдіть тест'); ?></a>
            <?= ($lang ? ' и узнайте' : ' і дізнайтеся'); ?><br class="hidden-sm hidden-xs">
            <strong><?= ($lang ? 'наиболее подходящее для вас направление в IT.' : 'найбільш підходящий для вас напрям в IT.'); ?></strong>
        </p>
        <style>
            .text-test-try {
                margin-bottom: 20px;
...
<!--        <li data-filter=".mobile">Mobile development</li>-->
    ...
    <?php $categories = getListRoadmaps(); ?>
    <div class="container_12 isotope <?php echo count($categories);?>">
        <?php
...
function prefix_generate_resume() {
    if ( wp_verify_nonce($_POST['verification'],'ITEA_of_the_best!') )
    {
        ...
        if ($creating_resume) {
            ...
            $message .= '<a href="https://itea.ua/resume/?id='.$uniqid.'" style="text-decoration: none; color: #fefefe; display: block" target="_blank">Перейти к Вашему резюме</a></span>';
